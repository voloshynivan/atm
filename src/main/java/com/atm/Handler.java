...
public class Handler {
    ...
    NewBalance balance = new NewBalance();
    ...
    public void handleInput(String userInput) {
        ...
        if (userInput.startsWith("-")) handleMinus(userInput);
            ...
            System.out.println("Sorry, you have type the command in incorrect format. Please make sure it is started from \"+\" or \"-\"");
    ...
    private void handleMinus(String userInput) {
        if (!userInput.matches("\\-\\s[A-Z]{3}\\s\\d+")) {
            ...
            if (MoneyBox.getInstance().getTotalMoneyForCurrency(currency) >= amount) {
                balance.getCards(currency, amount, MoneyBox.getInstance());
                System.out.println("OK");
            } else {
